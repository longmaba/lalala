...
/**
 * The oscilloscope generates a float array with array[i] an x coordinate in
 * percentage array[i+1] the value of the amplitude in audio buffer array[i+2]
 * another x coordinate in percentage array[i+3] the next amplitude in the audio
 * buffer
 * <p/>
 * The implementation is based on the one by Dan Ellis found at
...
public class Oscilloscope implements AudioProcessor {
  ...
  public static interface OscilloscopeEventHandler {
    ...
    /**
     * @param data The data contains a float array with: array[i] an x
     * coordinate in percentage array[i+1] the value of the amplitude in audio
     * buffer array[i+2] another x coordinate in percentage array[i+3] the next
     * amplitude in the audio buffer
