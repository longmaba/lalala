...
/**
 * An audio event flows through the processing pipeline. The object is reused
 * for performance reasons. The arrays with audio information are also reused,
 * so watch out when using the buffer getter and setters.
 *
 * @author Joren Six
 */
public class AudioEvent {
  ...
  /**
   * The number of bytes processed before this event. It can be used to
   * calculate the time stamp for when this event started.
   */
   ...
   *
   * @return a percentage of processed frames or a negative number if the number
   * of frames is not known beforehand.
   */
  ...
  /**
   * Return a byte array with the audio data in bytes. A conversion is done from
   * float, cache accordingly on the other side...
   *
  ...
  /**
   * Calculates and returns the root mean square of the signal. Please cache the
   * result since it is calculated every time.
   *
   * @return The <a href="http://en.wikipedia.org/wiki/Root_mean_square">RMS</a>
   * of the signal present in the current buffer.
   */
