...
/**
 * Agent is the central class for beat tracking. Each Agent object has a tempo
 * hypothesis, a history of tracked beats, and a score evaluating the
 * continuity, regularity and salience of its beat track.
 */
public class Agent {
  ...
  /**
   * The maximum amount by which a beat can be later than the predicted beat
   * time, expressed as a fraction of the beat period.
   */
  public static double POST_MARGIN_FACTOR = 0.3;

  /**
   * The maximum amount by which a beat can be earlier than the predicted beat
   * time, expressed as a fraction of the beat period.
   */
  public static double PRE_MARGIN_FACTOR = 0.15;

  /**
   * The default value of innerMargin, which is the maximum time (in seconds)
   * that a beat can deviate from the predicted beat time without a fork
   * occurring.
   */
  public static final double INNER_MARGIN = 0.040;

  /**
   * The maximum allowed deviation from the initial tempo, expressed as a
   * fraction of the initial beat period.
   */
  public static double MAX_CHANGE = 0.2;

  /**
   * The slope of the penalty function for onsets which do not coincide
   * precisely with predicted beat times.
   */
  public static double CONF_FACTOR = 0.5;

  /**
   * The reactiveness/inertia balance, i.e. degree of change in the tempo, is
   * controlled by the correctionFactor variable. This constant defines its
   * default value, which currently is not subsequently changed. The beat period
   * is updated by the reciprocal of the correctionFactor multiplied by the
   * difference between the predicted beat time and matching onset.
   */
  public static final double DEFAULT_CORRECTION_FACTOR = 50.0;

  /**
   * The default value of expiryTime, which is the time (in seconds) after which
   * an Agent that has no Event matching its beat predictions will be destroyed.
   */
  ...
  /**
   * The maximum time (in seconds) that a beat can deviate from the predicted
   * beat time without a fork occurring (i.e. a 2nd Agent being created).
   */
  protected static double innerMargin;

  /**
   * Controls the reactiveness/inertia balance, i.e. degree of change in the
   * tempo. The beat period is updated by the reciprocal of the correctionFactor
   * multiplied by the difference between the predicted beat time and matching
   * onset.
   */
  protected static double correctionFactor;

  /**
   * The time (in seconds) after which an Agent that has no Event matching its
   * beat predictions will be destroyed.
   */
  protected static double expiryTime;

  /**
   * For scoring Agents in a (non-existent) real-time version (otherwise not
   * used).
   */
  ...
  /**
   * Sum of salience values of the Events which have been interpreted as beats
   * by this Agent, weighted by their nearness to the predicted beat times.
   */
  public double phaseScore;

  /**
   * How long has this agent been the best? For real-time version; otherwise not
   * used.
   */
  ...
  /**
   * The current tempo hypothesis of the Agent, expressed as the beat period in
   * seconds.
   */
  public double beatInterval;

  /**
   * The initial tempo hypothesis of the Agent, expressed as the beat period in
   * seconds.
   */
  ...
  /**
   * The list of Events (onsets) accepted by this Agent as beats, plus
   * interpolated beats.
   */
   ...
   *
   * @param ibi The beat period (inter-beat interval) of the Agent's tempo
   * hypothesis.
   */
  ...
  /**
   * Output debugging information about this Agent, at the default (highest)
   * level of detail.
   */
  public void print() {
    print(100);
  ...
  /**
   * Accept a new Event as a beat time, and update the state of the Agent
   * accordingly.
   *
   ...
   * situations can occur: 1) The Agent has no beats yet; the Event is accepted
   * as the first beat. 2) The Event is beyond expiryTime seconds after the
   * Agent's last 'confirming' beat; the Agent is terminated. 3) The Event is
   * within the innerMargin of the beat prediction; it is accepted as a beat. 4)
   * The Event is within the outerMargin's of the beat prediction; it is
   * accepted as a beat by this Agent, and a new Agent is created which doesn't
   * accept it as a beat. 5) The Event is ignored because it is outside the
   * windows around the Agent's predicted beat time.
   *
   * @param e The Event to be tested
   * @param a The list of all agents, which is updated if a new agent is
   * created.
   * @return Indicate whether the given Event was accepted as a beat by this
   * Agent.
   */
  ...
  /**
   * Interpolates missing beats in the Agent's beat track, starting from the
  ...
  /**
   * Show detailed debugging output describing the beat tracking behaviour of
   * this agent. Calls showTracking()/1 with a default metrical level of 1.
   *
   ...
   *
   * @param allEvents An EventList of all onsets
   * @param level The metrical level of beat tracking relative to the notated
   * beat (used to count beats)
   */
