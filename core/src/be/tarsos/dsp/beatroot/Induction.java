...
/**
 * Performs tempo induction by finding clusters of similar inter-onset intervals
 * (IOIs), ranking them according to the number of intervals and relationships
 * between them, and returning a set of tempo hypotheses for initialising the
...
public class Induction {
  ...
  /**
   * The minimum inter-beat interval (IBI), i.e. the maximum tempo hypothesis
   * that can be returned. 0.30 seconds == 200 BPM 0.25 seconds == 240 BPM
   */
  public static double minIBI = 0.3;

  /**
   * The maximum inter-beat interval (IBI), i.e. the minimum tempo hypothesis
   * that can be returned. 1.00 seconds == 60 BPM 0.75 seconds == 80 BPM 0.60
   * seconds == 100 BPM
   */
      ...
      while (ptr2.hasNext()) {
        ...
        for (b = 0; b < intervals; b++) // assign to nearest cluster
        {
          if (Math.abs(clusterMean[b] - ioi) < clusterWidth) {
            if ((b < intervals - 1) && (Math.abs(clusterMean[b + 1] - ioi)
  ...
  /**
   * An alternative (incomplete) tempo induction method (not used). Uses integer
