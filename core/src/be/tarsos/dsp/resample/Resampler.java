 ...
 * ****************************************************************************
 * <p/>
 * libresample4j Copyright (c) 2009 Laszlo Systems, Inc. All Rights Reserved.
 * <p/>
 * libresample4j is a Java port of Dominic Mazzoni's libresample 0.1.3, which is
 * in turn based on Julius Smith's Resample 1.7 library.
...
public class Resampler {
  ...
  /**
   * Clone an existing resampling session. Faster than creating one from
   * scratch.
   *
   * @param other
   */
   ...
   * @throws IllegalArgumentException if minFactor or maxFactor is not positive,
   * or if maxFactor is less than minFactor
   */
  ...
  /**
   * Process a batch of samples. There is no guarantee that the input buffer
   * will be drained.
   *
   ...
   * @return true iff resampling is complete (ie. no input samples consumed and
   * no output samples produced)
   */
  ...
  /**
   * Process a batch of samples. Convenience method for when the input and
   * output are both floats.
   *
   * @param factor factor at which to resample this batch
  ...
  /**
   * Process a batch of samples. Alternative interface if you prefer to work
   * with arrays.
   *
