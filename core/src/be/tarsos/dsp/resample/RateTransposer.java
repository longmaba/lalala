...
public class RateTransposer implements AudioProcessor {
   ...
   * rate. E.g. 0.5 is half the sample rate, 1.0 does not change a thing and 2.0
   * doubles the samplerate. If the samples are played at the original speed the
   * pitch doubles (0.5), does not change (1.0) or halves (0.5) respectively.
   * Playback length follows the same rules, obviously.
   *
   * @param factor Determines the new sample rate. E.g. 0.5 is half the sample
   * rate, 1.0 does not change a thing and 2.0 doubles the sample rate. If the
   * samples are played at the original speed the pitch doubles (0.5), does not
   * change (1.0) or halves (0.5) respectively. Playback length follows the same
   * rules, obviously.
   */
