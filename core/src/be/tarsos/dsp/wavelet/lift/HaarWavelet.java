...
public class HaarWavelet extends LiftingSchemeBaseWavelet {
   ...
   * </p>
   * <p>
   * The wavelet transform calculates a set of detail or difference coefficients
   * in the predict step. These are stored in the upper half of the array. The
   * update step calculates an average from the even-odd element pairs. The
   * averages will replace the even elements in the lower half of the array.
   * </p>
   * <p>
   ...
   * <
   * pre>
   * d<sub>j+1, i</sub> = odd<sub>j+1, i</sub> = odd<sub>j, i</sub> -
   * even<sub>j, i</sub>
   * a<sub>j+1, i</sub> = even<sub>j, i</sub> = (even<sub>j, i</sub> +
   * odd<sub>j, i</sub>)/2
   * </pre>
   * <p>
