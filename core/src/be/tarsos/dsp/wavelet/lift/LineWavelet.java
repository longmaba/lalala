...
public class LineWavelet extends LiftingSchemeBaseWavelet {
   ...
   * the end of the data series. Here we pretend that the last two values in the
   * data series are at the x-axis coordinates 0 and 1, respectively. We then
   * need to calculate the y-axis value at the x-axis coordinate 2. This point
   * lies on a LineWavelet running through the points at 0 and 1.
   * </p>
   * <p>
   ...
   * (y - y<sub>1</sub>) = -------- (x - x<sub>1</sub>) . x<sub>2</sub> -
   * x<sub>1</sub>
   * </pre>
   * <p>
   * Solving for y
   ...
   * <p/>
   * <p>
   * The predict step attempts to "predict" the value of an odd element from the
   * even elements. The difference between the prediction and the actual element
   * is stored as a wavelet coefficient.
   * </p>
   * <p>
   ...
   * <
   * pre>
   * a<sub>0</sub>, a<sub>1</sub>, a<sub>1</sub>, a<sub>3</sub>, b<sub>0</sub>,
   * b<sub>1</sub>, b<sub>2</sub>, b<sub>2</sub>,
   * </pre>
   * <p>
   * The predict step of the LineWavelet wavelet "predicts" that the odd element
   * will be on a LineWavelet between two even elements.
   * </p>
   * <p/>
   * <
   * pre>
   * b<sub>j+1,i</sub> = b<sub>j,i</sub> - (a<sub>j,i</sub> +
   * a<sub>j,i+1</sub>)/2
   * </pre>
   * <p>
   * Note that when we get to the end of the data series the odd element is the
   * last element in the data series (remember, wavelet algorithms work on data
   * series with 2<sup>n</sup> elements). Here we "predict" that the odd element
   * will be on a LineWavelet that runs through the last two even elements. This
   * can be calculated by assuming that the last two even elements are located
   * at x-axis coordinates 0 and 1, respectively. The odd element will be at 2.
   * The <i>new_y()</i> function is called to do this simple calculation.
   * </p>
   */
   ...
   * array. The update phase works on the even elements in the first half of the
   * array. The update phase attempts to preserve the average. After the update
   * phase is completed the average of the even elements should be approximately
   * the same as the average of the input data set from the previous iteration.
   * The result of the update phase becomes the input for the next iteration.
   * </p>
   * <p>
   ...
   * element (e.g., its odd neighbor before the split). This is not possible in
   * the LineWavelet wavelet since the odd element has been replaced by the
   ...
   * Sweldens' journal articles and his tutorial (<i>Building Your Own Wavelets
   * at Home</i>) and in <i>Ripples in Mathematics</i>. A somewhat more complete
   * derivation of this equation is provided in <i>Ripples in Mathematics</i> by
   * A. Jensen and A. la Cour-Harbo, Springer, 2001.
   * </p>
   * <p>
   ...
   * <
   * pre>
   * even<sub>i+1,j</sub> = even<sub>i,j</sub> op (odd<sub>i+1,k-1</sub> +
   * odd<sub>i+1,k</sub>)/4
   * </pre>
   * <p>
