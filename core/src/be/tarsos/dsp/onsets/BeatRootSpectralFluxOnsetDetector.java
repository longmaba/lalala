...
public class BeatRootSpectralFluxOnsetDetector implements AudioProcessor, OnsetDetector {
  ...
  /**
   * The real part of the data for the in-place FFT computation. Since input
   * data is real, this initially contains the input data.
   */
  private float[] reBuffer;

  /**
   * The imaginary part of the data for the in-place FFT computation. Since
   * input data is real, this initially contains zeros.
   */
  ...
  /**
   * A mapping function for mapping FFT bins to final frequency bins. The
   * mapping is linear (1-1) until the resolution reaches 2 points per semitone,
   * then logarithmic with a semitone resolution. e.g. for 44.1kHz sampling rate
   * and fftSize of 2048 (46ms), bin spacing is 21.5Hz, which is mapped linearly
   * for bins 0-34 (0 to 732Hz), and logarithmically for the remaining bins
   * (midi notes 79 to 127, bins 35 to 83), where all energy above note 127 is
   * mapped into the final bin.
   */
  private int[] freqMap;

  /**
   * The number of entries in <code>freqMap</code>. Note that the length of the
   * array is greater, because its size is not known at creation time.
   */
  private int freqMapSize;

  /**
   * The magnitude spectrum of the most recent frame. Used for calculating the
   * spectral flux.
   */
  ...
  /**
   * For dynamic range compression, this value is added to the log magnitude in
   * each frequency bin and any remaining negative values are then set to zero.
   */
  ...
  /**
   * Ratio between rate of sampling the signal energy (for the amplitude
   * envelope) and the hop size
   */
  ...
  /**
   * Creates a map of FFT frequency bins to comparison bins. Where the spacing
   * of FFT bins is less than 0.5 semitones, the mapping is one to one. Where
   * the spacing is greater than 0.5 semitones, the FFT energy is mapped into
   * semitone-wide bins. No scaling is performed; that is the energy is summed
