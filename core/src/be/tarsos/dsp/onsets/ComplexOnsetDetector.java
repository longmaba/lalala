...
public class ComplexOnsetDetector implements AudioProcessor, OnsetDetector {
   ...
   * @param peakThreshold A threshold used for peak picking. Values between 0.1
   * and 0.8. Default is 0.3, if too many onsets are detected adjust to 0.4 or
   * 0.5.
   * @param silenceThreshold The threshold that defines when a buffer is silent.
   * Default is -70dBSPL. -90 is also used.
   * @param minimumInterOnsetInterval The minimum inter-onset-interval in
   * seconds. When two onsets are detected within this interval the last one
   * does not count. Default is 0.004 seconds.
   */
