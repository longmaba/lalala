 ...
 * of data in a sound stream. By examing the information stored in the audio
 * format, you can discover how to interpret the bits in the binary sound data.
 * <p/>
 * Every data LineWavelet has an audio format associated with its data stream.
 * The audio format of a source (playback) data LineWavelet indicates what kind
 * of data the data LineWavelet expects to receive for output. For a target
 * (capture) data LineWavelet, the audio format specifies the kind of the data
 * that can be read from the LineWavelet. Sound files also have audio formats,
 * of course.
 * <p/>
 * The <code>AudioFormat</code> class accommodates a number of common sound-file
 ...
 * uses is named by its <code>encoding</code> field.
 * <p/>
 ...
 * the associated property value is an arbitrary object. Properties specify
 * additional format specifications, like the bit rate for compressed formats.
 * Properties are mainly used as a means to transport additional information of
 * the audio format to and from the service providers. Therefore, properties are
 * ignored in the AudioFormat method.
 * <p/>
 * <p/>
...
public class TarsosDSPAudioFormat {
   ...
   * @param bigEndian indicates whether the data for a single sample is stored
   * in big-endian byte order (<code>false</code> means little-endian)
   */
   ...
   * @param bigEndian indicates whether the data for a single sample is stored
   * in big-endian byte order (<code>false</code> means little-endian)
   * @param properties a <code>Map&lt;String,Object&gt;</code> object containing
   * format properties
   * @since 1.5
   */
   ...
   * given parameters. The frame size is set to the number of bytes required to
   * contain one sample from each channel, and the frame rate is set to the
   * sample rate.
   *
   * @param sampleRate the number of samples per second
   * @param sampleSizeInBits the number of bits in each sample
   ...
   * for queries capabilities , a sample rate of
   ...
   * used for queries or capabilities , a sample size of
   ...
   * or capabilities , a return value of <code>AudioSystem.NOT_SPECIFIED</code>
   * means that any (positive) number of channels is acceptable.
   *
   ...
   * or capabilities, a frame size of <code>AudioSystem.NOT_SPECIFIED</code>
   * means that any frame size is acceptable.
   * <code>AudioSystem.NOT_SPECIFIED</code> is also returned when the frame size
  ...
  /**
   * Obtains the frame rate in frames per second. When this AudioFormat is used
   * for queries or capabilities , a frame rate of
  ...
  /**
   * Indicates whether the audio data is stored in big-endian or little-endian
   * byte order. If the sample size is not more than one byte, the return value
   * is irrelevant.
   *
   ...
   * properties. If no properties are recognized, an empty map is returned.
   ...
   * also have the same sample rate, unless the specified format has the sample
   * rate value <code>AudioSystem.NOT_SPECIFIED</code>, which any sample rate
   * will match. The frame rates must similarly be equal, unless the specified
   * format has the frame rate value <code>AudioSystem.NOT_SPECIFIED</code>. The
   * byte order (big-endian or little-endian) must match if the sample size is
  ...
  /**
   * The <code>Encoding</code> class names the specific type of data
   * representation used for an audio stream. The encoding includes aspects of
   * the sound format other than the number of channels, sample rate, sample
   * size, frame rate, frame size, and byte order.
   * <p/>
   * One ubiquitous type of audio encoding is pulse-code modulation (PCM), which
   * is simply a linear (proportional) representation of the sound waveform.
   * With PCM, the number stored in each sample is proportional to the
   * instantaneous amplitude of the sound pressure at that point in time. The
   * numbers are frequently signed or unsigned integers. Besides PCM, other
   * encodings include mu-law and a-law, which are nonlinear mappings of the
   ...
   * created by this class, such as PCM_SIGNED or PCM_UNSIGNED. Service
   * providers can create new encodings, such as compressed audio formats or
   * floating-point PCM samples, and make these available through the
   * <code>AudioSystem</code> class.
   * <p/>
   * The <code>Encoding</code> class is static, so that all
   * <code>AudioFormat</code> objects that have the same encoding will refer to
   * the same object (rather than different instances of the same class). This
   * allows matches to be made by checking that two format's encodings are
   * equal.
   *
   * @author Kara Kytle
   * @since 1.3
  ...
  public static class Encoding {
     ...
     * variable (field) name. For example, <code>PCM_SIGNED.toString()</code>
