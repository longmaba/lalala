 ...
 * to <code>O((W+w)log(W+w))</code> by using an <abbr title="Fast Fourier
 * Transform">FFT</abbr> to calculate the <abbr title="Auto-Correlation
 * Function">ACF</abbr>. But I am still afraid of the dark magic of the FFT and
 * clinging to the familiar, friendly, laggard time domain.
 * </p>
 *
...
public final class McLeodPitchMethod implements PitchDetector {
   ...
   * the first peak that is higher than 93% of the highest peak detected. 93% is
   * the default value used in the Tartini user interface.
   */
   ...
   * sample rate.
   *
   * @param audioSampleRate The sample rate of the audio to check.
   */
   ...
   *
   * @param audioSampleRate The sample rate of the audio.
   * @param audioBufferSize The size of one audio buffer 1024 samples is common.
   */
   ...
   * @param audioSampleRate The sample rate of the audio.
   * @param audioBufferSize The size of one audio buffer 1024 samples is common.
   * @param cutoffMPM The cutoff (similar to the YIN threshold). In the Tartini
   * paper 0.93 is used.
   */
   ...
   * </p>
   * <p>
   * a,b,c are three samples that follow each other. E.g. a is at 511, b at 512
   * and c at 513; f(a), f(b) and f(c) are the normalized square difference
   * values for those samples; x is the peak of the parabola and is what we are
   * looking for. Because the samples follow each other <code>b - a = 1</code>
   * the formula for <a href="http://fizyka.umk.pl/nrbook/c10-2.pdf">parabolic
   * interpolation</a>
   * can be simplified a lot.
   * </p>
   * <p>
   * The following ASCII ART shows it a bit more clear, imagine this to be a bit
   * more curvaceous.
   * </p>
   * <p/>
   * <
   * pre>
   * nsdf(x) ^ | f(x) |------ ^ f(b) | / |\ f(a) | / | \ | / | \ | / | \ f(c) |
   * / | \ |_____________________> x a x b c
   * </pre>
   *
   * @param tau The delay tau, b value in the drawing is the tau value.
   */
   ...
   * Including the highest value between the last positive zero crossing and the
   * end (if any). Ignoring the first maximum (which is at zero). In this
   * diagram the desired values are marked with a +
   * </p>
   * <p/>
   * <
   * pre>
   * f(x) ^ | 1| + | \ + /\ + /\ 0| _\____/\____/__\/\__/\____/_______> x | \ /
   * \ / \/ \ / -1| \/ \/ \/ |
   * </pre>
   *
