...
public strictfp class FloatFFT {
   ...
   * <
   * pre>
   * a[2*k] = Re[k], 0&lt;=k&lt;(n+1)/2 a[2*k+1] = Im[k], 0&lt;k&lt;(n-1)/2 a[1]
   * = Im[(n-1)/2]
   * </pre>
   * <p/>
  ...
  /**
   * Computes 1D forward DFT of real data leaving the result in <code>a</code> .
   * This method computes the full real forward transform, i.e. you will get the
   * same result as from <code>complexForward</code> called with all imaginary
   * parts equal 0. Because the result is stored in <code>a</code>, the size of
   * the input array must greater or equal 2*n, with only the first n elements
   ...
   * This method computes the full real forward transform, i.e. you will get the
   * same result as from <code>complexForward</code> called with all imaginary
   * part equal 0. Because the result is stored in <code>a</code>, the size of
   * the input array must greater or equal 2*n, with only the first n elements
   * filled with real data. To get back the original data, use
   * <code>complexInverse</code> on the output of this method.
   *
  ...
  /**
   * Computes 1D inverse DFT of real data leaving the result in <code>a</code> .
   * This method computes the full real inverse transform, i.e. you will get the
