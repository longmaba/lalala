 ...
 * switch FFT algorithm with minimal overhead. Support for window functions is
 * also present.
 *
 * @author Joren Six
 */
public final class FFT {
  ...
  /**
   * Create a new fft of the specified size. Apply the specified window on the
   * samples before a forward transform. arning: the window is not applied in
   * reverse when a backwards transform is requested.
   *
   * @param size The size of the fft.
   * @param windowFunction Apply the specified window on the samples before a
   * forward transform. arning: the window is not applied in reverse when a
   * backwards transform is requested.
   */
  ...
  /**
   * Computes inverse DFT. Warning, does not reverse the window function.
   *
   * @param data data to transform
   */
   ...
   * magnitude or absolute value is (a²+b²) ^ 0.5 with a being the real part and
   ...
   *
   * @param data The FFT transformed data.
   * @param power The array where the magnitudes or powers are going to be
   * stored. It is half the length of data (FFT size).
   * @param phase The array where the phases are going to be stored. It is half
   * the length of data (FFT size).
   */
