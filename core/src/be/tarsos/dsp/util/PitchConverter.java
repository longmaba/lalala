...
public final class PitchConverter {
   ...
   * range every pitch is mapped to a MIDI key. If a value outside the range is
   ...
   * @throws IllegalArgumentException if the hertzValue does not fall within the
   * range of valid MIDI key frequencies.
   */
   ...
   * @throws IllegalArgumentException If midiKey is not in the valid range
   ...
   * IllegalArgumentException ("Relative cent values are values from 0 to 1199,
   * inclusive " + relativeCent + " is invalid."); int defaultOctave = 5; int
   * offset = defaultOctave * 1200; return absoluteCentToHertz(relativeCent +
   * offset); }
   */
  /**
  ...
  /**
   * Returns the frequency (Hz) of an absolute cent value. This calculation uses
   * a configured reference frequency.
   *
