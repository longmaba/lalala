 ...
 * <blockquote> An envelope detector is an electronic circuit that takes a
 * high-frequency signal as input and provides an output which is the envelope
 * of the original signal. The capacitor in the circuit stores up charge on the
 * rising edge, and releases it slowly through the resistor when the signal
 * falls. The diode in series rectifies the incoming signal, allowing current
 * flow only when the positive input terminal is at a higher potential than the
 * negative input terminal.
 * </blockquote>
 * <p/>
 * The resulting envelope is stored in the buffer in the processed AudioEvent.
 * The class can be used thusly:
 * <p/>
 * <
 * pre>
 * EnvelopeFollower follower = new EnvelopeFollower(44100);
 *
 * AudioDispatcher dispatcher = AudioDispatcher.fromFloatArray(sine, 44100,
 * 1024, 0);
 *
 *
 * dispatcher.addAudioProcessor(follower); dispatcher.addAudioProcessor(new
 * AudioProcessor() {
 *
 * public boolean process(AudioEvent audioEvent) { //envelope float buffer[] =
 * audioEvent.getFloatBuffer(); for(int i = 0 ; i < buffer.length ; i++){
 * System.out.println(buffer[i]); } return true; }
 *
 * public void processingFinished() { } }); dispatcher.run(); </pre>
 *
 * @author Joren Six
 */
public class EnvelopeFollower implements AudioProcessor {
   ...
   * @param releaseTime Defines how fast the envelope goes down, defined in
   * seconds.
   */
